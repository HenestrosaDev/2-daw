...
export class Equipo {
	_capitalizeFirstLetter = string => string[0].toUpperCase() + string.slice(1);
	...
	_checkAJugadores = _aJugadores => {
		const aJugadoresFiltered = _aJugadores.filter((obj) => obj instanceof Jugador);
	...
	_checkEntrenador = entrenador => {
		if (!entrenador || typeof entrenador !== "string") {
			const message = "El nombre del entrenador tiene que ser de tipo 'string'. No puede estar vacío.";
			printMessage(message);
			throw TypeError(message);
		} else if (entrenador.length < 4) {
			const message = "El nombre del entrenador tiene que contener 4 o más caracteres.";
			printMessage(message);
	...
	constructor(nombreEquipo, ciudad, escudo, entrenador) {
		...
		this._ciudad =
			this._capitalizeFirstLetter(ciudad.toLocaleLowerCase());
