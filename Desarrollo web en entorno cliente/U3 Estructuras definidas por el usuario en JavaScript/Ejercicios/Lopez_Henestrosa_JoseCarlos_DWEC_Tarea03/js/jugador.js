...
export class Jugador extends Participante {
	...
	_checkFechaNac = fechaNac => {
		...
		if (!isDate && !(this._isDateWithinRange(from, to, fechaNac))) {
			const message = "La fecha de nacimiento tiene que estar comprendida entre el 01/01/1979 y el 31/12/2011.";
	...
	_filterDorsal = dorsal => {
		if (typeof dorsal !== "number") {
			const message = "El dorsal tiene que ser de tipo 'number'. Asegúrate de que no va entre comillas.";
			printMessage(message);
