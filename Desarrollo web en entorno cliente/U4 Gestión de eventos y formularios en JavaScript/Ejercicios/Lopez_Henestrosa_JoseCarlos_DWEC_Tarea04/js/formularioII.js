 ...
 * @returns true si está todo en orden, false
 * si hay algún campo no válido.
 */
const areInputsValid = () => {
	...
	for (const el of document.getElementById("lecture__form").querySelectorAll("[required]")) {
		if (!el.checkValidity()) {
			el.reportValidity()
			return false;
		}
...
/**
 * Muestra el resultado en el div con id "form__feedback"
 * de los inputs rellenados del formulario.
 */
const showResult = () => {
	appendElement("Resultados", '');
	for (const el of document.getElementById("lecture__form").querySelectorAll("input")) {
		appendElement(el.labels[0].innerText, el.value);
	}
	// Como <textarea> no es un elemento <input>, tenemos que mostrarlo fuera del bucle
	appendElement(
		document.getElementById("lecture__description").labels[0].innerText,
		document.getElementById("lecture__description").value
	);
	// Como <select> no es un elemento <input>, tenemos que mostrarlo fuera del bucle
	appendElement(
		document.getElementById("lecture__category").getAttribute("aria-label"), 
		document.getElementById("lecture__category").value
	);

	// Quitamos el display:none del CSS y mostramos el div con los elementos p añadidos
