...
const showPrompt = () => {
	var sentence = prompt(
		"Introduce la frase a encriptar. Ten en cuenta que tiene que tener más de 4 palabras."
	);
	sentence.trim();
...
const checkPrompt = (sentence) => {
	if (countWords(sentence) <= 4) {
		...
		// Mostramos la ventana hasta que el usuario introduzca una respuesta 
		// válida o cierre el prompt
		showPrompt();
	} else {
...
class Encryptor {
	...
	getNormalEncryption() {
		let sentenceInAscii = [];
		// Iteramos por cada carácter contenido en la entrada del usuario
	...
	getRandomEncryption() {
		...
		for (let char of this.sentence) {
			...
			// Comprobamos que el valor encriptado no sea superior al valor máximo 
			// en ASCII
			if (result > maxAscii) {
				// Si se pasa de 255, restamos el valor ASCII base y realizamos la
				// operación indicada en el ejercicio
...
const showEncryptedSentences = (sentence) => {
	...
	let encryptions = Array.from({ length: 3 }, () =>
	...
	encryptions[2].innerHTML = `<strong>El mensaje en ASCII sumando el número aleatorio ${
		encryptor.random
	}</strong>: ${encryptor.getRandomEncryption()}`;
