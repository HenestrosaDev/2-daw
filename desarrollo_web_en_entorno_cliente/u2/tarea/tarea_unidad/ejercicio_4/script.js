...
const inputLegs = () => {
	...
	// Eliminamos aquellos caracteres que no sean números o ";", el cual será el
	// carácter que indique la separación entre las medidas
...
const showPrompt = () =>
	prompt(
		"Introduce los dos catetos en este formato: 'X; Y' donde 'X' será el primer cateto e 'Y' el segundo."
	);
...
const confirmInput = (legs) => {
	if (
		confirm(
			`El primer cateto es ${legs[0]} y el segundo es ${legs[1]}. ¿Deseas continuar?`
		)
	) {
		printResults(legs);
	} else {
...
const printResults = (legs) => {
	...
	result.textContent = `La hipotenusa del cateto1 = ${legs[0]} y el cateto2 = ${
		legs[1]
	} es ${calculateHypotenuse(legs).toFixed(2)}`;
