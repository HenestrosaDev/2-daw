	...
	<div class="container-fluid">
		...
		<div class="collapse navbar-collapse" id="navbarCollapse">
			<ul class="navbar-nav me-auto mb-2 mb-md-0">
				<li class="nav-item">
					<a class="nav-link <?= (!empty($active) && $active == 1) ? "active" : "" ?>" href="<?= $relative . load_page("1_test") ?>">
						Test
					</a>
				</li>
				<li class="nav-item">
					<a class="nav-link <?= (!empty($active) && $active == 2) ? "active" : "" ?>" href="<?= $relative . load_page("2_reservas") ?>">
						Reserva de coches
					</a>
				</li>
				<li class="nav-item">
					<a class="nav-link <?= (!empty($active) && $active == 3) ? "active" : "" ?>" href="<?= $relative . load_page("3_pizzeria") ?>">
						Pizzería
					</a>
				</li>
			</ul>
			<ul class="navbar-nav">
				...
				<?php if (!empty($_SESSION["is_logged_in"]) && $_SESSION["is_logged_in"] == true) { ?>
					<li class="nav-item">
						<button type="button" class="btn <?= (($_SESSION["user_role"] === 'U') ? "btn-outline-light" : "btn-outline-warning") ?>" style="opacity: 1 !important;" disabled>Registrado como: <?= $_SESSION["username"] ?></button>
					</li>
					<li class="nav-item">
						<a type="button" id="button--logout" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#logout__modal">Cerrar sesión</a>
					</li>
				<?php } else { ?>
					<li class="nav-item">
						<a type="button" id="button--sign_up" class="btn btn-primary btn-success" data-bs-toggle="modal" data-bs-target="#sign_up__modal">Regístrate</a>
					</li>
					<li class="nav-item">
