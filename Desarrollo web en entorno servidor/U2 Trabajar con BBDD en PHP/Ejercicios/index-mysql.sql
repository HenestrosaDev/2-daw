...
--------------------------------------------- 
*/
DROP TRIGGER IF EXISTS tr_attempt_ai;
...
CREATE TRIGGER tr_attempt_ai
  AFTER INSERT ON attempt FOR EACH ROW 
	BEGIN
		...
		SET 
			average = (SELECT FORMAT(AVG(score), 2) FROM attempt),
			mode_statistic =	(
				SELECT score
				FROM (
					SELECT score
			...
			variance = (SELECT FORMAT(VARIANCE(score), 2) FROM attempt),
			standard_deviation = (SELECT FORMAT(STD(score), 2) FROM attempt);
	END$$
