...
const insertIngredient = (ingredientId, ingredientName) => {
	...
	$("#checkout__total").text(
		round(parseFloat($("#checkout__total").text()) + 0.8)
	);

	toggleIngredientsNumber();
};

const generatePizzaId = () =>
	`custom_${
...
$("#dought__toggle").on("click", () => {
	if ($("#dought__price").text() == 3.95) {
		...
		$("#checkout__total").text(
			round(parseFloat($("#checkout__total").text()) + 1)
		);

		// Sustituimos el tipo de masa en el array del checkout
		checkout[pizzaId] = checkout[pizzaId].map((item) =>
			item.id == "medium-dought" ? { id: "familiar-dought", price: 4.95 } : item
		);
		...
		// Sustituimos el tipo de masa en el array del checkout
		checkout[pizzaId] = checkout[pizzaId].map((item) =>
			item.id == "familiar-dought" ? { id: "medium-dought", price: 3.95 } : item
		);
	}
});
...
$("#checkout__submit").on("click", () => {
	$.post(
		"./php/check_checkout.php",
		{ checkout: checkout[pizzaId] },
		(response) => {
...
$(document).on("click", "[id*='__remove--checkout']", (e) => {
	...
	checkout[pizzaId] = checkout[pizzaId].filter(
		(item) => item.id !== ingredient[0]
	);
