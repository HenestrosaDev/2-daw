...
function toggleTheme(shouldChange) {
	...
	document
		.querySelector(".form__results")
		.classList.toggle("form__results--light-theme");
}

function check(input) {
	if (input.validity.typeMismatch) {
		input.setCustomValidity(
			"'" + input.value + "' no es un número español válido."
		);
	} else {
		input.setCustomValidity("");
	}
...
function addTextField(amount, placeholder, name, id, generated) {
	...
	var new_input =
		"<input type='text' class='form__item form__item--generated' placeholder='" +
		placeholder +
		" (adicional)' name='" +
		name +
		new_chq_no +
